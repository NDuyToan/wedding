...
export default function WeddingInfoSection() {
  return (
    <section className="bg-[#fff7f4] px-6 py-24">
      <div className="mx-auto max-w-6xl">
        ...
        >
          <p className="mb-3 text-sm font-semibold uppercase tracking-[0.38em] text-rose-400">
            Trân trọng kính mời
          </p>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_1.08fr]">
          <div className="space-y-6">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-1">
              {families.map((family, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="border border-rose-100 bg-white p-7 shadow-sm"
                >
            ...
            >
              ...
              <p className="wedding-script text-5xl leading-tight text-[#9b475d]">
                Nguyễn Duy Toản
              </p>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="border border-rose-100 bg-white p-6 shadow-sm md:p-8"
          >
            <div className="mb-8 flex flex-col gap-3 text-center md:flex-row md:items-end md:justify-between md:text-left">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.34em] text-rose-500">
                  Tháng 05 - 2026
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-7 gap-2 text-center">
              ...
              {calendarCells.map((cell) => (
                <div
                  key={cell.key}
                  className="relative flex aspect-square items-center justify-center overflow-hidden border border-rose-50 bg-[#fffaf8] text-sm font-semibold text-stone-600"
                >
                  {cell.type === "day" && (
                    <>
                      {cell.highlight && (
                        <span className="absolute text-5xl leading-none text-rose-400">
                          ♥
                        </span>
                      )}
                      <span
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2">
              {weddingDays.map((event) => (
                <div key={event.label} className="border border-rose-100 p-5">
                  <p className="mb-2 text-xs font-bold uppercase tracking-[0.28em] text-rose-500">
                    {event.label}
                  </p>
